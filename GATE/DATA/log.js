...
const delOlds=(dir,min,one=false)=>{//오래된 로그파일을 삭제한다.//대량데이터시 작동여부 확인필요.
  let thisTime=+ new Date();
  fs.readdir(dir,(e,files)=>{
    if(!e){
      files.sort();//.list.txt 파일이 가장 먼저 온다.
      let cliStr='';
      let filesCnt=files.length;
      //console.log(files,filesCnt);
      if(filesCnt>1) for(let i in files){
        //console.log(i);
        let file=files[i];
        let stat=fs.statSync(dir+file);
        let time=+ new Date(stat.ctime);

        if(one && Number(i)<filesCnt-1){//하나만 남기고 모두 삭제하기
          cliStr='rm -rf '+dir+file;
          //console.log(cliStr,i);
          cliCmd(cliStr,()=>{
            //console.log(dir+file,'deleted olds files');
            makeList(dir);
          });
        }
        else if(thisTime-time>=1000*60*min){
          cliStr='rm -rf '+dir+file;
          //console.log(cliStr);
          cliCmd(cliStr,()=>{
            //console.log(dir+file,'deleted olds files');
          ...
          //console.log(cliStr,thisTime-time,'>=',1000*60*min);
        }
...
const newLogDir=(logLimitMinute)=>{
  //로그를 생성하는 시점에서 디렉토리를 생성할 경우, 여러개의 클러스터가 충돌할 우려가 있다.
  //또한 디렉토리가 존재하는 여부를 검사하는 로직도 추가되어 무겁다.
  //따라서 디렉토리는 별도로 관리하되, 필요한 디렉토리를 주기적으로 선행해서 생성하여 불필요한 탐색을 줄여야 한다.
  //현재시간을 구하고, 그 시점에서 미래의 디렉토리를 미리 만들어둔다.
  //지나간 시점의 디렉토리는 주어진 제한에 따라 주기적으로 삭제한다.
  let {path}=getLogDir('');

  let thisTime=+ new Date();
  //console.log(thisTime);

  let minPlus=2;//미리 생성할 디렉토리//분당 하나씩, 디렉토리를 미리 생성한다.
  //1개의 디렉토리만 미리 생성할 경우, 생성하는 시간 동안 충돌이 일어날 수 있다.
  for(let mn=0;mn<minPlus;mn++){
    let min1=1000*60;
    let tTime=thisTime+(min1*minPlus)-(min1*mn);
    let {thisYMD,thisHM}=mkLogDate(tTime);

    let pathDate=path+thisYMD+'/';
    let pathHm=pathDate+thisHM+'/';
    //console.log(mn,pathHm);
...
const delLogOld=(base,fnc,mins=60*24)=>{
  ...
  if(days>0){//날짜별로 제거, 즉 최소 어제+오늘 치를 보관한다.
    ...
    list=getLogList(path);
    //console.log(list.length,'days',list[list.length-1],'~',list[0]);//삭제하는데 시간이 걸리므로 정확하지 않다
  }//

  else{//오늘의 디렉토리만 보관하고, 분단위로 오래된 것을 삭제한다.
    delOlds(path,1,true);//마지막 하나만 남기기
    //20220415//오늘만//어제 이전은 모두 삭제된다.
    //list=getLogList(path);
    //console.log(list.length,'dirs (d)',list[list.length-1],'~',list[0]);//삭제하는데 시간이 걸리므로 정확하지 않다

    delOlds(pathDate,mins);
    //0643,0644,0645...//여러시분
    list=getLogList(pathDate);
    //console.log(list.length,'mins',list[list.length-1],'~',list[0]);//삭제하는데 시간이 걸리므로 정확하지 않다
  }
}
...
module.exports={
  ...
  newLogDir:newLogDir,//로그디렉토리생성-시간기준자동생성
